...
{{-- <script>
    ...
    $(".brandfilter").on("click", function() {
        brand = getCheckedValues(".brandfilter");
        fetchProducts(brand)
    });

    $(".availability").on("click", function() {
        availability = getCheckedValues(".availability");
        fetchProducts(availability)
    });


    function fetchProducts(page = 1, brand, availability) {

        console.log(brand, availability);

        $.ajax({
            url: "{{ url($category->category_slug) }}" + "?page=" + page,
            type: "GET",
            data: {
                brand: brand,
                availability: availability
